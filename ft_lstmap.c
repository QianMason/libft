...
/*   Updated: 2019/03/12 17:15:14 by mqian            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
** returns NULL
** allocates mem for new elem. if it fails at any point, return NULL; wont there be memory left dangling if it allocates some and then it doesnt allocate one and returns NULL?
** if (!track) line is to handle for first time you create elem
** sets head and track to curr. curr moves to next elem;
** every other time the loop points the element befores next to the curr that was just created.
** then curr moves and track moves too;
** lst is moved at the end of the loop to get the next element of the list we are referencing
